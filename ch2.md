...
本章内容
- 对本书 [第 3 章](ch3.md) 至 [第 6 章](ch6.md) 中介绍编译技术的总体介绍
- 通过开发一个可运行Java程序演示这些编译技术
- 可以将具有代表性的程序设计语言翻译为三地址代码
- 重点是编译器的前端，特别是词法分析/语法分析/中间代码生成
- [第 7 章](ch7.md) 和 [第 8 章](ch8.md) 将介绍如何根据三地址代码生成机器指令
...
- 在2.3节中，我们将介绍一种面向文法的翻译技术，__语法制导翻译__（syntax-directed translation）\
  *（应该是翻译的问题，应该翻译成“语法导向翻译”。）*\
  *（我搜索了“制导”，它是指引导飞行器；这里并没有什么东西在飞，而应该是在说，由“语法”指引“翻译”的发展方向，即“导向”）*
...
- 利用 `expr` 的结构，用如下伪代码翻译 `expr`\
  ...
  处理 +;
  ```
- 我们将在 [2.8 节](ch2.md#生成中间代码) 中使用这段伪代码的一个变体，为 `expr` 构造一颗语法分析树
...
- __翻译方案__（translation scheme）：一种将程序片段附加到一个文法的各个表达式上的表示法
  ...
  *（说得太过晦涩了，不过这里是在往 __文法__ 里面加东西，简单来说往里面加“程序片段”，被称为“语义动作”。实际上在后面的具体小节中还会解释的。）*

语法制导翻译方案将在本章多次使用
- 把中缀表达式翻译成后缀表达式
- 用于表达式求值，并用来构建一些程序构造的抽象语法树
- [第 5 章](ch5.md) 将更详细地讨论语法制导表示法
...
*（第一次看听起来很晦涩，但是一旦读到后面的例子之后回过头来看，可能会清楚一些。它的意思简单来说是在往 __文法__ 里面加东西，也就是附加一种叫“属性”的值。而且满足上面这段话描述的“属性”还叫“综合属性”）*
...
- 如果某个属性在语法分析树结点 N 上的值是由 N 的子节点以及N本身的属性值确定的\
  *（这句话非常奇怪，分析树结点 N 上属性值，有一部分还决定于自身？为以防万一，我还找到了英语原文）*
...
*（为什么？ __翻译方案__ 一定要用/等同于后序遍历吗？）*\
*（这里说“实现一个翻译方案”时，并没有限定得到的是“后缀表达式”。可能意思是说，实现翻译方案，__语义动作__ 插入的位置一定要排在子节点之间的最后）*
...
- 在标号为 `optexpr` 的非终结符号结点上，我们需要再次为一个非终结符号选择产生式，以 ε 为体的产生式（称 ε 产生式）需要特别处理
  ...
  - 我们将在 [2.4.3 节](ch2.md#何时使用-ε-产生式) 中再次讨论 ε 产生式
