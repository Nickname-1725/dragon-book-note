...
## 向前看运算符
- Lex 自动向前读入一个字符
- 读取到形成被选词素全部字符之后的那个字符，然后再回退输入

限定某个模式后面必须有特定字符
- 某些时候，我们希望仅当词素的后面跟随特定的其它字符时，这个词素才能和某个特定的模式相匹配
- 可以在模式中用斜线来指明该模式中和词素实际匹配的部分的结尾
- 只有附加模式和输入匹配后，才可以确定已经看到了要寻找的词法单元的词素\
  *（说得不是很具体，实际上指的是 [3.3.6 节](ch3.md#33-节的练习) 中定义的扩展的正则表达式）*
- 但是和第二个模式匹配的字符并不是这个词素的一部分

例子：Fortran 和一些其它语言中，关键字并不是保留字
- 比如 `IF(I,J)=3`，`IF` 是一个数组的名字，而不是关键字
- 形成对比的是这样形式的语句 `IF ( condition ) THEN ...`
- 可以确定关键字 `IF`
  - 后面总是跟着一个左括号
  - 然后是一些可能包含在括号中的文本（条件表达式）
  - 接着是一个右括号和一个字母
- 可以为关键字 `IF` 写出 Lex 规则：`IF / \( . * \) |letter|`

局限
- 为了使模式简单可靠我们必须对输入进行预处理：消除其中的空白符
- 该模式不能处理条件表达式跨行的情形，因为点号不能和一个换行符匹配
...
本节揭示 Lex 如何将它的输入程序变成一个词法分析器，核心是被称为 __有穷自动机__（finite automata）的表示方法

这些自动机本质上是与 __状态转换图__ 类似的图，但有如下几点不同
1. 有穷自动机是 __识别器__（recognizer），只能对每个可能的输入串简单地回答“是”/“否”
2. 有穷自动机分为两类
    1. __不确定的有穷自动机__（Nondeterministic Finite Automata, NFA）
        - 对其边上的标号没有任何限制
        - 一个符号标记离开同一状态的多条边
        - 且空串 ε 也可以作为标号
    2. __确定的有穷自动机__（Deterministic Finite Automata,DFA）
        - 对于每个状态及自动机输入字母表的每个符号，有且仅有一条离开该状态、以该符号为标号的边

确定的和不确定的有穷自动机能识别的语言的集合是相同的
- 这些语言的集合正好是能用正则表达式描述的语言集合
- 这个集合中的语言称为 __正则语言__（regular language）\
  *（作者注：有个小问题，按照我们的定义，正则表达不能描述空的语言，因为实践中从不会使用这样的模式；但是，有穷自动机可以定义空语言。理论研究中，Φ 被视为一个额外的正则表达式，它的用途就是定义空语言）*

## 不确定的有穷自动机
不确定的有穷自动机（NFA）组成
1. 有穷的状态集合 S
2. 一个输入符号集合 Σ，即 __输入字母表__（input alphabet）；假设代表空串的 ε 不是 Σ 中的元素
3. 一个 __转换函数__（transition function），为每个状态和 $\Sigma \cup\{\epsilon\}$ 中的每个符号都给出了相应的 __后继状态__ 的集合
4. S 中的一个状态 s0 被指定为 __开始状态__/__初始状态__
5. S 中的一个子集 F 被指定为 __接受状态__/__终止状态__ 的集合

不管是 NFA 还是 DFA，我们都可以将它表示为一张 __转换图__（transition graph）
- 结点：状态
- 带标号的边：自动机的转换函数
- 从状态 s 到状态 t 存在一条标号为 a 的边，当且仅当：状态 t 是状态 s 在输入 a 上的后继状态之一

这个图和 __状态转换图__ 十分相似，不同之处
1. 同一个符号可以标记从同一状态出发到达多个目标状态的多条边
2. 同一条边的标号不仅可以是输入字母表中的符号，也可以是空符号串 ε

*（一个比较直观的例子就是，前面说的，如果不把关键字当保留字，那么同一个模式匹配到关键字和标识符都有可能）*

例子：一个能够识别正则表达式 `( a|b ) * abb` 的语言的 NFA 的转换图
- 这个例子将贯穿本节；虽然很抽象，但是实际上它和一些具有实际意义的正则表达式相似
- 沿用 __状态转换图__ 中的惯例
  ```
  ─ start ─ (0)
            ├─ a ── (0)
...
## 转换表
将一个NFA表示为一张 __转换表__（transition table）
- 各行对应于状态
- 各列对应于输入符号和 ε
- 对应于一个给定状态和输入，查取的条目，是将 NFA 的转换函数应用于这些参数后得到的值
- 如果转换函数没有给出对应于某 __状态-输入对__ 的信息，就把 Φ 放入相应的表项中
...
| 3 | Φ | Φ | Φ |
优点
- 很容易确定一个给定状态、一个输入符号所对应的转换

缺点
- 如果输入字母表很大，且大多数状态在大多数输入字符上没有转换，转换表需要占用大量空间

## 自动机中输入字符串的接受
一个 NFA 接受（accept）输入字符串 x，当且仅当
- 对应的转换图中存在一条从开始状态到某个接受状态的路径
- 使得该路径中各条边上的标号组成符号串 x

补充说明：路径中的 ε 标号将被忽略，因为空串不会影响到根据路径构建得到的符号串

例子：*略*

注意：
- 只要存在 __某条__ 其标号序列为符号串的路径，能够从开始状态到达某个接受状态，NFA 就接受这个符号串
- 存在某些到达非接受状态的路径并不会影响这个结论

由一个 NFA __定义__/__接受__ 的语言：从开始状态到某个接受状态的所有路径上的标号串的集合

我们可以用 L(A) 表示自动机 A 接受的语言

例子：*略*

## 确定的有穷自动机
__确定的有穷自动机__（DFA）：__不确定有穷自动机__ 的一个特例，并且
1. 没有输入 ε 之上的转换动作
2. 对每个状态 s 和每个输入符号 a，有且只有一条标号为 a 的边离开 s

在构造词法分析器的时候，我们真正实现或模拟的是 DFA
- 每个正则表达式和每个 NFA 都可以被转换成一个接受相同语言的 DFA

*（翻译的时候出了问题，贴一下中文版原文和英文版原文）*
> 幸运的是，每个正则表达式和每个 DFA 都可以被转变称为一个接受相同语言的 DFA
...
正则表达式非常适合描述词法分析器和其它模式处理软件，然而那些软件的实现需要模拟 DFA/NFA 的执行
- 由于 NFA
  - 对于一个输入符号可以选择不同的转换
  - 还可以执行输入 ε 上的转换
  - 甚至可以选择是对 ε 还是对真实输入符号执行转换
- 因此对 NFA 的模拟不如对 DFA 的模拟直接
- 我们需要将一个 NFA 转换为一个识别相同语言的 DFA

本节预告
- 先介绍如何把 NFA 转化为 DFA
- 然后，我们利用这种称为 __子集构造法__ 的技术给出一个直接模拟 NFA 的算法，可用于那些将 NFA 转化到 DFA 比直接模拟 NFA 更加耗时（非词法分析的）的情形
- 说明如何把正则表达式转换为 NFA，在必要时根据这个 NFA 构造出一个 DFA
- 讨论不同的正则表达式实现技术之间的时间-空间权衡问题，如何为具体的应用选择合适的方法

## 从 NFA 到 DFA 的转换
__子集构造法__ 的基本思想
- 让构造得到的 DFA 的每个状态对应于 NFA 的一个状态集合
- DFA 在读入输入 $a_1a_2\cdots a_n$ 之后达到的状态对应于相应 NFA 从开始状态出发，沿着 $a_1a_2\cdots a_n$ 为标号的路径能够达到的状态的集合

状态数的问题
- DFA 的状态数有可能是 NFA 状态数的指数，这种情况下，我们在试图实现 DFA 时会遇到困呐
- 然而，基于词法分析方法的处理能力部分源于如下事实
  - 对于一个真实的语言，它的 NFA 和 DFA 的状态数量大致相同
  - 状态数量呈指数关系的情形尚未在实践中出现过

算法：由 NFA 构造 DFA 的 __子集构造__（subset construction）算法
- 输入：一个 NFA N
- 输出：一个接受同样语言的 DFA D
- 方法：
  - 算法为 D 构造一个转换表 Dtran
  - D 的每个状态是一个 NFA 状态集合
  - 构造 Dtran 使得 D “并行地”模拟 N 在遇到一个给定输入时可能执行的所有动作
...
| ε-closure(T) | 能够从 T 的某个 NFA 状态 s 开始只通过 ε 转换到达的 NFA 状态集合，即 $\cup_{s\in T}\epsilon-closure(s)$ |
| move(T,a) | 能够从 T 的某个状态 s 出发通过标号为 a 的转换到达的 NFA 状态的集合 |
- 这些函数描述了一些需要在这个算法中执行 N 的状态集上的基本操作
- s 表示 N 的单个状态，T 代表 N 的一个状态机

找出当 N 读入了某个输入串之后可能位于的所有状态集合
1. 在读入第一个符号之前，N 可以位于集合 $\epsilon-closure(s_0)$ 中的任何状态上，其中 s0 是 N 的开始状态
2. 开始归纳
    - 假定 N 在读入输入串 x 之后可以位于集合 T 中的状态上
    - 如果下一个输入符号是 a，那么 N 可以立即移动到 move(T,a) 中的任何状态
    - 然而，N 也可以在读入 a 后再执行几个 ε 转换，因此 N 在读入 xa 之后可位于 $\epsilon-closure(move(T,a))$ 中的任何状态上\
    *（原来如此，这样一来上面定义的三个函数就都使用到了）*
3. 根据这些思想，得到方法
    ```
    一开始，ε-closure(s0) 是 Dstates 中的唯一状态，且它未加标记;
...
只需要说明如何对 NFA 的任何状态集合 T 计算 $\epsilon-closure(T)$，就可以完整地描述子集构造法

例子：*略*

将在 [3.9.6 节] 中讨论使一个 DFA 的状态个数最小化的问题

## NFA 的模拟
很多文本编辑程序使用的策略是
- 根据一个正则表达式构造出相应的 NFA
- 使用类似于 on-the-fly（边构造边使用）的子集构造法来模拟这个 NFA 的执行

算法：模拟一个 NFA 的执行
- 输入
  - 一个以文件结束符 eof 结尾的输入串 s
  - 一个 NFA N，其开始状态为 s0，接受状态集为 F，转换函数为 move
- 输出
  - 如果 N 接受 x 则返回“yes”，否则返回“no”
- 方法
  - 保存一个当前状态的集合 S，那些可以从 s0 开始沿着标号为当前已读入输入部分的路径到达的状态的集合
  - 如果 c 是函数 nextChar() 读到的下一个输入字符，那么我们首先计算 move(S,c)，然后使用 ε-clusure 求出这个集合的闭包
...
```
*（算法思想和 __子集构造法__ 一脉相承，相当于每一步都考虑到所有接下来可能的状态；直到消耗完输入字符串之后，看有没有某些状态是被接受的）*

## NFA 模拟的效率
如果精心实现，模拟 NFA 算法的效率可以相当高效，这些高效实现的思想可以用于许多涉及图搜索的算法

需要的数据结构
1. 两个堆栈
    - 每一个堆栈都存放了一个 NFA 状态集合
    - 其中的一个堆栈 `oldStates` 存放“当前状态集合”，即刚才算法伪代码的第 4 行右边 S 的值
    - 另一个堆栈 `newStates` 存放“下一个”状态集合，即伪代码第 4 行左边 S 的值
    - 在我们运行第 3 行到第 6 行的循环时，中间的一个步骤没有列出，即把 `newStates` 的值转移到 `oldStates` 中去的步骤
2. 一个以 NFA 状态为下标的布尔数组 `alreadyOn`
    - 指示哪个状态已经在 `newStates` 中
    - 虽然这个数组存放的信息和栈中存放的信息相同，但查询 `alreadyOn[s]` 要比在栈 `newStates` 中查询 s 快很多
3. `二维数组 move[s,a]`
    - 保存这个 NFA 的转换表
    - 条目是状态的集合，用链表表示

- 为了实现伪代码的第一行
  - 将 `alreadyOn` 数组中的所有条目都设置为 `FALSE`
  - 对于 `ε-closure(s0)` 中的每个状态 s，将 s 压入 `oldStates` 并设置 `alreadyOn[s]` 为 `TRUE`
- 对状态 s 的操作以及伪代码第 4 行中的操作，都可以使用函数 `addState(s)` 来实现
  - 这个函数将 s 压入 `newStates`，将 `alreadyOn[s]` 设置为 `TRUE`
  - 使用 `move[s,ε]` 作为参数递归调用自身，继续计算 `ε-closure(s)` 的值
  - 为了避免重复工作，必须小心，不要对一个已经在栈 `newStates` 中的状态调用 `addState`
  ...
  ```
- 通过查看 `oldStates` 中的每个状态 s 来实现伪代码的第四行
  ``` C
  for (oldStates 上的每个 s) {
    for (move[s,c] 中的每个 t)
      if (!alreadyOn[t])
        addState(t);
    将 s 弹出 oldStates 栈;
  ...
  - 找出状态集合 `move[s,c]`
  - c 是下一个输入字符
  - 对于不在 `newStates` 栈中的状态，应用函数 `addState`
  - 注意，`addState` 还计算了一个状态的 ε-closure 值，并把其中的状态一起加入到 `newStates` 中

时间消耗
- 假定一个 NFA N 有 n 个状态和 m 个转换，即 m 是离开各个状态的转换数的总和
- 如果不包括（伪代码第 4 行的具体实现）对 `addState` 的调用，在第 1 个循环上花费的时间是 O(n)
- 对于第 2 个循环，这个结论也成立

时间复杂度分析：*略*

结论：
- 只要实现方法得当，执行伪代码第 4 行的时间是 O(n+m)
- 那么该循环的总工作量为 O(k(n+m))
- 也就是说，该算法所需时间和输入串的长度、转换图的大小的乘积成正比

<div style="border:1px solid;padding">
...
## 从正则表达式构造 NFA
给出一个算法，将任何正则表达式转变为接受相同语言的 NFA
- 这个算法是语法导向的，也就是说它沿着正则表达式的语法分析树自底向上递归地处理\
  *（我担心的事情还是发生了……正则表达式的实现本身也需要实现“编译器”，也需要编译原理）*
- 对于每个子表达式，该算法构造一个只有一个接受状态的 NFA

算法：将正则表达式转换为一个 NFA 的 McMaughton-Yamada-Thompson 算法
- 输入：字母表 Σ 上的一个正则表达式 r
- 输出：一个接受 L(r) 的 NFA N
- 方法
  - 对 r 进行语法分析，分解出组成它的子表达式
  - 构造一个 NFA 的规则
    - 基本规则：处理不包括运算符的子表达式
    - 归纳规则：根据一个给定表达式的直接子表达式的 NFA 构造出这个表达式的 NFA
...
- 对于表达式 ε，构造下面的 NFA
  ...
  ─ start ─ (i)
            └─ ε ── ((f))
  ```
  - i 是一个新状态，也是 NFA 的开始状态
  - f 是另一个新状态，也是 NFA 的接受状态
- 对于字母表 Σ 中的子表达式，构造下面的 NFA
  ```
  ─ start ─ (i)
            └─ a ── ((f))
  ```
  - i 和 f 都是新状态，分别是 NFA 的开始状态和接受状态
- 这两个基本构造规则中，对于 ε 或某个 a 的作为 r 的子表达式的每次出现，都会使用新状态分别构造出一个独立的 NFA

__归纳规则__：假设正则表达式 s 和 t 的 NFA 分别为 N(s) 和 N(t)
1. 假设 r=s|t，r 的 NFA，即 N(r)，可以按照下面的方式构造
    ```
    ─ start ─ (i)
              ├─ ε ── [N(s)]
              │       └─ ε ── ((f))
              └─ ε ── [N(t)]
                      └─ ε ── ((f))
    ```
    - i 和 f 是新状态，分别是 N(r) 的开始状态和接受状态
    - 从 i 到 N(s) 和 N(t) 的开始状态各有一个 ε 转换
    - 从 N(s) 和 N(t) 到接受状态 f 也各有一个 ε 转换
    - 从 i 到 f 的任何路径要么只通过 N(s)，要么只通过 N(t)
2. 假设 r = st，然后按照下面的方式构造 N(r)
    ```
    ─ start ─ (i)[N(t)]()[N(t)] ((f))
    ```
    - N(s) 的接受状态和 N(t) 的开始状态合并为一个状态，合并后的状态拥有原来进入和离开合并前的两个状态的全部转换
3. 假设 r = s*，然后为 r 构造出下面的 NFA N(r)
    ```
    ─ start ─ (i)
              ├─ ε ── [N(s)]
              │       ├─ ε ── [N(s)]
              ...
              └─ ε ── ((f))
    ```
    - i 和 f 是两个新状态，分别是 N(r) 的开始状态和唯一的接受状态
4. 假设 r = (s)，那么L(r)=L(s)，我们可以直接把 N(s) 当作 N(r)

由该算法构造得到的 NFA 所具有的性质
1. N(r) 的状态数最多为 r 中出现的运算符和运算分量的总数的 2 倍，原因是算法的每一个构造最多只引入两个新状态
2. N(r) 有且只有一个开始状态和一个接受状态
3. N(r) 中除接受状态之外的每个状态要么有一条其标号为 Σ 中符号的出边，要么有两条标号为 ε 的出边

例子：*略*

## 字符串处理算法的效率
问题的产生
- [3.6.3 小节](ch3.md#自动机中输入字符串的接受) 中模拟一个 DFA 的算法能在 O(|x|) 时间内处理字符串 x
- [3.7.3 小节](ch3.md#nfa-模拟的效率) 中提到模拟一个 NFA 运行所需的时间与 |x| 和该 NFA 的转换图的大小的乘积成正比
- 很显然，用 DFA 来模拟比用 NFA 模拟更快，因此我们可能会怀疑模拟一个 NFA 到底有没有意义

例子：形如 $L_n=(a|b)^*a(a|b)^{n-1}$ 的正则表达式描述的语言组
- 很容易构造出一个具有 n+1 个状态的 NFA
- 然而，Ln 的任何一个 DFA 都至少有 $2^n$ 个状态
- 说明：*略*
- 幸运的是，正如前面提到的，词法分析很少需要处理这种类型的模式，也不用担心会遇到状态数量出奇多的 DFA

将正则表达式转换成 DFA 还是 NFA 的问题：词法分析工具/其它字符串处理系统经常以正则表达式作为输入
- 转换成 DFA 的额外开销是应用 [第 3.7.1小节](ch3.md#从-nfa-到-dfa-的转换) 中的由 NFA 构造 DFA 的 __子集构造算法__ 的开销（也可以将一个正则表达式直接转化为 DFA，但工作实质上是一样的）
- 如果字符串被处理器频繁使用，比如词法分析器，那么转到 DFA 时付出的任何代价都是值得的
- 在另一些字符串处理应用（如 grep）中，用户指定正则表达式，并在一个或多个文件中搜索表达式所描述的模式，跳过 DFA 直接模拟 NFA 可能更加高效

考虑用 [3.7.4 小节](ch3.md#从正则表达式构造-nfa) 中将正则表达式转换为 NFA 的 McMaughton-Yamada-Thompson 算法，将正则表达式 r 转换成 NFA 的代价
- 关键不愁是构造 r 的语法分析树，[第 4 章](ch4.md) 中有几种可以在线性时间内构造语法分析树的方法，即在 O(|r|) 时间内完成语法分析上述的构造，|r| 表示 r 的大小
- 很容易发现每次应用 McMaughton-Yamada-Thompson 算法的基本规则和归纳规则只需要常数时间
- 转换得到 NFA 花费的全部时间是 O(|r|)

模拟 NFA 的代价
- 如我们在 [3.7.4 节](ch3.md#从正则表达式构造-nfa) 中观察到的，构造 NFA 最多有 2|r| 个状态和 4|r| 个转换
- 根据 [3.7.3 节](ch3.md#nfa-模拟的效率) 中的分析，可以得到 n≤2|r| 和 m≤4|r|
- 模拟这个 NFA 处理输入字符串 x 的过程所花费的时间是 $O(|r|\times|x|)$
- 远远超过构造 NFA 所用的时间 O(|r|)
- 对于正则表达式 r 和字符串 x，能够在 $O(|r|\times|x|)$ 时间内判断 x 是否属于 L(r)

子集构造法所花费的时间很大程度上取决于构造得到的 DFA 的状态数。我们已经知道，如果实现得当，这个步骤所花的时间最多和 NFA 状态数与转换数之和成正比
- 假设从正则表达式 r 开始，构造成一个 NFA，这个 NFA 最多有 2|r| 个状态和 4|r| 个转换，最多有 2|r| 个输入符号
- 对于每个构造得到的 DFA 状态，最多必须构造 |r| 个新状态，构造每个新状态最多花费 O(2|r|+4|r|) 时间
- 构造一个有 s 个状态的 DFA 所用时间为 $O(|r|^2s)$

通常情况与最坏情况
- 通常情况：s 大于等于 |r|，上面的子集构造法需要的时间为 $O(|r|^3)$
- 然而，在最快情况下，这个时间是 $O(|r|^22^{|r|})$
- 当我们需要构造一个识别器指明一个或多个串 x 是否存在一个给定的正则表达式 r 所定义的 L(r) 时，有多个选项

| 自动机 | 初始开销 | 每个串的开销 |
|-|-|-|
| NFA | $O(\|r\|)$ | $O(\|r\|\times\|x\|)$ |
| DFA 典型情况 | $O(\|r\|^3)$ | $O(\|x\|)$ |
| DFA 最坏情况 | $O(\|r\|^22^{\|r\|})$ | $O(\|x\|)$ |
- 词法构造器的情况：处理各个字符串所花的时间多很多，倾向于使用 DFA
- grep 这样的命令：只会对一个符号串运行这个自动机，倾向于使用 NFA 方式，只有当 |x| 接近 $|r|^3$ 的时候，我们才会考虑转换到 DFA

__混合策略__：对于每个正则表达式 r 和输入串 x，效率总是和 DFA 和 NFA 方法中较好的一个差不多
- 从模拟 NFA 开始
- 在计算出各个状态集（DFA 的状态）和转换的同时把它们记录下来
- 模拟中每次处理此 NFA 的当前状态集合和当前输入符号之前，首先查看是否已经计算了这个转换，如果是，就直接使用
...
本节内容：
- 应用 [3.7 节](ch3.md#从正则表达式到自动机) 中介绍的技术
- 讨论像 Lex 这样的词法分析器生成工具的体系结构
- 讨论分别基于 NFA 和 DFA 的方法，后者实质上就是 Lex 的实现方法

## 生成的词法分析器的结构
由 Lex 生成的词法分析器的体系结构
- 包含一个固定的模拟自动机的程序（DFA/NFA）
- 这些组件包括
  1. 表示自动机的一个转换表
  2. 由 Lex 编译器从 Lex 程序中直接拷贝到输出文件的函数（[3.5.2 节](ch3.md#lex-程序的结构)）
  3. 输入程序定义的动作（一些代码片段）

构建自动机的规程
- 先用 [3.7.4 节](ch3.md#从正则表达式构造-nfa) 中构造 NFA 的算法将 Lex 程序中的每个正则表达式模式转换为一个 NFA
- 将这些 NFA 合并为一个 NFAs
- 合并的方法是引入一个新的开始状态，从这个新开始状态到各个对应于模式 pi 的 NFA Ni 的开始状态各有一个 ε 转换

例子：略

## 基于 NFA 的模式匹配
如果词法分析器模拟了一个合并后的 NFA
- 它必须从它的输入中 `lexemeBigin` 所至的位置开始读取输入
- 当它在输入中向前移动 `forward` 指针时，它在每个位置上根据 [3.7.2 节](ch3.md#nfa-的模拟) 中的模拟 NFA 运行的算法计算当前的状态集
- 这个模拟 NFA 运行的过程中，最终会到达一个没有后续状态的输入点，此时不可能有任何更长的输入前缀使得 NFA 到达某个接受状态

判定最长前缀（与某模式匹配对应的词素）
- 沿着状态集的顺序回头寻找，直到找到一个包含一个或多个接受状态的集合位置
- 如果集合中有多个接受状态，就选择和在 Lex 程序中位置最靠前的模式相关联的那个接受状态 pi
- 将 `forward` 指针移回到词素末尾，执行与 pi 相关联的动作 Ai

*（也就是说，还需要回溯？是不是需要/有没有可能用函数栈来模拟这种回溯？）*

例子：*略*

## 词法分析器使用的 DFA
使用 [3.7.1 节](ch3.md#从-nfa-到-dfa-的转换) 中将 NFA 转换为 DFA 的算法，将所有模式的 NFA 转化为等价的 DFA
- DFA 的每个状态中，如果该状态包含一个或多个 NFA 的接受状态
- 就要确定哪些模式的接受状态出现在此 DFA 状态中，并找出第一个这样的模式
- 将该模式作为这个 DFA 状态的输出

例子：*略*

词法分析器中，使用 DFA 的方法与使用 NFA 的方法很相似
- 模拟这个 DFA 的运行，直到在某个点上没有后续状态为止（严格来说下一个状态为 Φ，对应于空的 NFA 状态集合的 __死状态__）
- 回头查找我们进入过的状态序列，一旦找到接受状态就执行与该状态对应的模式相关联的动作

例子：*略*

## 实现向前看运算符
回顾 [3.5.4 节](ch3.md#向前看运算符)，Lex 模式 r1/r2 中的 Lex 向前看运算符 `/` 必不可少\
*（没错，我也有这样一个疑问，之前并没有实现过向前看运算符）*
- 将模式 r1/r2 转化成 NFA 时，把 `/` 看成 ε，实际上不会在输入中查找 `/`
- 如果 NFA 发现输入缓冲区的一个前缀 xy 和这个正则表达式匹配，这个词素的末尾并不在 NFA 进入接受状态的地方
- 而是在状态 s，满足以下条件
  1. s 在（假想的）`/` 上有一个 ε 转换
...
假想 `/` 上 ε 转换状态的个数
- 如果 NFA 中只有1个在 `/` 上的 ε 转换状态，词素的末尾出现在最后一次进入该状态的地方
- 如果 NFA 在假想的 `/` 上有多个 ε 转换状态，那么如何寻找正确的状态 s 的问题就会变得困难得多

*（不太明白为什么会有多个 ε 转换状态？是指只有一个 ε 是真正由 `/` 引入的，其它的都是来自另外一些原因的吗？这样一来，就需要确定哪个 ε 是在 `/` 的位置上，对吗？）*
...
**DFA 中的死状态**
- 严格来说，[3.8.3 节](ch3.md#词法分析器使用的-dfa) 例子中提到的 DFA 并不是一个真正的 DFA
- 因为 DFA 中每个状态在它的输入字母表中的每个符号上都有一个离开转换
- 我们省略了到达 __死状态__ Φ 的转换
- 也省略了从这个死状态出发、在所有输入符号上到达其自身的转换

处理死状态的方式
- 当我们构造一个用于词法分析器的 DFA 时，必须用不同的方式处理死状态
- 我们必须知道什么时候已经不可能识别到更长的词素了
- 建议省略到达死状态的转换，并消除死状态本身
- 这个问题比看起来要困难，因为一个 NFA 到 DFA 的构造过程中可能产生多个不可能到达接受状态的 DFA 状态；必须知道何时到达了这样一个状态
- [3.9.6 节] 讨论了如何将这些状态合并为一个死状态，使得识别这些状态变得容易
- 如果使用
  - [3.7.1](ch3.md#从-nfa-到-dfa-的转换) 中由 NFA 构造 DFA 的子集构造算法
  - [3.7.4 节](ch3.md#从正则表达式构造-nfa) 中将正则表达式转换为 NFA 的算法
  
  根据正则表达式构造出 DFA，得到的 DFA 中除 Φ 之外所有的状态都可到达某个接受状态
...
本节给出 3 个算法，用于实现和优化根据正则表达式构造得到的模式匹配器
1. 第 1 个算法：用于 Lex 编译器，它不需要构造中间的 NFA 就可以根据一个正则表达式直接构造得到 DFA；得到的 DFA 的状态数也比通过 NFA 构造得到的 DFA 状态数少
2. 第 2 个算法：将任何 DFA 中具有相同未来行为的多个状态合并，使该 DFA 的状态数量减到最少
3. 第 3 个算法：生成比标准二维表更加紧凑的转换表的表示方式

*（这三个算法看着太厉害了，为什么不早点搬出来，前面废了那么大功夫）*

## NFA 的重要状态

## 根据抽象语法树计算得到的函数

## 计算 nullable、firstpos 及 lastpos
